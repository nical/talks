(function () {
  ...
  function showIFrameIfAuto(slideNumber) {
    // hide the iframe before
    if (slides[slideNumber-1] &&
        slides[slideNumber-1].className.indexOf('autoiframe')!== -1) {
      var frame = slides[slideNumber - 1].querySelector('iframe');
      if (frame) {
        console.log('hide iframe');
        frame.src = "";
      }
    }
    // hide the iframe after
    if (slides[slideNumber+1] &&
        slides[slideNumber+1].className.indexOf('autoiframe')!== +1) {
      var frame = slides[slideNumber + 1].querySelector('iframe');
      if (frame) {
    ...
    // show the current slide's iframe
    if (slides[slideNumber] &&
        slides[slideNumber].className.indexOf('autoiframe')!== -1) {
      var frame = slides[slideNumber].querySelector('iframe');
      if (frame) {
